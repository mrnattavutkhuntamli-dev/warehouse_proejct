...
import { useCreateMaterial, useUpdateMaterial, useMaterialCategories } from "@/services";
...
const { register, handleSubmit, control, reset, formState: { errors } } = useForm({
  ...
  defaultValues: {
    ...
    description: "",
    unit:        "",
    categoryId:  defaultValues?.categoryId ?? defaultValues?.category?.id ?? "",  // ✅ คำนวณตรงนี้เลย
    minStock:    0,
    maxStock:    null,
    location:    "",
      ...
      <FormSection title="ข้อมูลพื้นฐาน">
        <FormRow>
          ...
          <FormField label="หน่วย" error={errors.unit?.message} required>
            <Input placeholder="ลิตร, ชิ้น, กก., ม้วน" error={errors.unit} {...register("unit")} />
          </FormField>
        </FormRow>

        <FormField label="ชื่อวัสดุ" error={errors.name?.message} required>
          <Input placeholder="น้ำมันเกียร์ ISO 68" error={errors.name} {...register("name")} />
        </FormField>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select value={field.value} onValueChange={field.onChange}>
                <SelectTrigger className={errors.categoryId ? "border-[var(--color-danger)]" : ""}>
        ...
        <FormField label="คำอธิบาย" error={errors.description?.message}>
          <Textarea placeholder="รายละเอียดเพิ่มเติม..." rows={3} {...register("description")} />
        </FormField>
      </FormSection>

      <FormSection title="สต็อก">
        <FormRow>
          <FormField label="สต็อกต่ำสุด (แจ้งเตือน)" error={errors.minStock?.message} required>
            <Input type="number" step="0.01" min="0" placeholder="0" error={errors.minStock} {...register("minStock")} />
          </FormField>
          <FormField label="สต็อกสูงสุด" error={errors.maxStock?.message} hint="เว้นว่างหากไม่ต้องการกำหนด">
            <Input type="number" step="0.01" min="0" placeholder="ไม่จำกัด" {...register("maxStock")} />
          </FormField>
        </FormRow>
      </FormSection>

      <FormActions onCancel={onCancel} loading={mutation.isPending} submitLabel={isEdit ? "บันทึกการแก้ไข" : "เพิ่มวัสดุ"} />
    </form>
  );
}
