...
export function GRForm({ defaultPoId, onSuccess, onCancel }) {
  ...
  const { data: poListData } = usePurchaseOrders({
    status: "APPROVED,PARTIAL_RECEIVED",
    limit: 200,
  });

  const suppliers = suppliersData ?? [];
  const locations = locationsData ?? [];
  const approvedPOs = poListData ?? [];
      ...
      <FormSection title="รายการวัสดุที่รับ">
        ...
        <div className="space-y-3">
          <div className="hidden sm:grid grid-cols-[1fr_90px_110px_150px_36px] gap-2 px-1">
            {["วัสดุ", "จำนวน", "ราคา/หน่วย", "เก็บที่ Location", ""].map(
              (h) => (
                <p
                  key={h}
                    ...
                    {selectedPO ? (
                      <div className="h-9 flex items-center px-3 rounded-md bg-[var(--color-surface-3)] border border-[var(--color-border)] gap-2">
                        ...
                        <span className="text-sm text-[var(--color-text-primary)] truncate">
                          {mat?.name ?? watchedItems?.[idx]?.materialId}
                        </span>
                      </div>
                    ) : (
                      <Input
                          ...
                          <SelectContent>
                            ...
                            {locations.map((loc) => (
                              <SelectItem key={loc.id} value={loc.id}>
                                <span className="font-mono text-xs text-[var(--color-brand)] mr-1">
                                  {loc.code}
                                </span>
                                {loc.warehouse?.name}
                              </SelectItem>
