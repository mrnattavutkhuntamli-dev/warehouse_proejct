...
export function IssueForm({ onSuccess, onCancel }) {
  ...
  for (const s of stockData ?? []) {
    stockByMaterial[s.materialId] =
      (stockByMaterial[s.materialId] ?? 0) + s.quantity;
  }
          ...
          {fields.map((field, idx) => {
            ...
            return (
              <div key={field.id} className="space-y-1">
                          ...
                          <SelectContent>
                            {materials.map((m) => (
                              <SelectItem key={m.id} value={m.id}>
                                <span className="font-mono text-xs text-[var(--color-brand)] mr-2">
                                  {m.code}
                                </span>
                                {m.name}
                ...
                {mat && stockQty !== null && (
                  <div
                    className={`flex items-center gap-2 text-xs font-mono px-1 ${insufficient ? "text-[var(--color-danger)]" : "text-[var(--color-text-muted)]"}`}
                  >
                    <span>
                      สต็อกปัจจุบัน: {formatNumber(stockQty, 2)} {mat.unit}
                    </span>
                    {insufficient && (
                      <span className="font-semibold">⚠ สต็อกไม่พอ</span>
                    )}
                  </div>
                )}
              </div>
