    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <FormSection title="ข้อมูล PO">
              ...
              <SelectContent>
                {suppliers.map((s) => (
                  <SelectItem key={s.id} value={s.id}>
                    {s.name} <span className="text-[var(--color-text-muted)] ml-1">({s.code})</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <FormField label="หมายเหตุ / เงื่อนไขพิเศษ">
          <Textarea placeholder="สั่งด่วน, เงื่อนไขการชำระเงิน, ฯลฯ" rows={2} {...register("note")} />
        </FormField>
      </FormSection>
                        ...
                        <SelectContent>
                          {materials.map((m) => (
                            <SelectItem key={m.id} value={m.id}>
                              <span className="font-mono text-[var(--color-brand)] mr-2">{m.code}</span>
                              {m.name}
      ...
      <FormActions onCancel={onCancel} loading={createMutation.isPending} submitLabel="สร้าง PO" />
    </form>
  );
}
